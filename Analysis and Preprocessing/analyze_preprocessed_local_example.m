...
sub_folder = fullfile(pwd,'preprocessed');
...
for run = 1
    ...
    for camera_ind = 2
        ...
        img_folder = fullfile(sub_folder,run_name{run},['camera' num2str(camera_ind)]);

        img_files = dir(fullfile(img_folder,'*.mat'));
        img_files = natsortfiles(img_files);
        load(fullfile(sub_folder,run_name{run},['camera' num2str(camera_ind)],'dark','dark_preprocessed.mat'));
...
load('analyzed\run1\run1camera2.mat')
ind = 1:length(cc);
t = ind/100*source_num;
examples_sources = [2 4];

figure; 
plot(t,cc(:,examples_sources));
xlabel('time [s]');
ylabel('Intensity [ADU]');
title('Intensity Window Average');

figure;
plot(t,Kraw2(:,examples_sources));
xlabel('time [s]');
ylabel('Contrast Squared')
title('Raw Contrast Squared Window Average')

figure;
tiledlayout(3,1)

nexttile;
plot(t,Kr2(:,examples_sources))
xlabel('time [s]');
ylabel('Contrast Squared')
title('Read Noise Contrast Squared Window Average')

nexttile;
plot(t,Ks2(:,examples_sources))
xlabel('time [s]');
ylabel('Contrast Squared')
title('Shot Noise Contrast Squared Window Average')

nexttile;
plot(t,Ksp2(:,examples_sources))
xlabel('time [s]');
ylabel('Contrast Squared')
title('Spatial Heterogineity Noise Contrast Squared Window Average')

figure;
tiledlayout(2,1)

nexttile
plot(t,Kf2(:,examples_sources))
xlabel('time [s]');
ylabel('Contrast Squared')
title('Fundemental Contrast Squared Window Average')

nexttile
rDb = 1./Kf2./mean(1./Kf2);
plot(t,rDb(:,examples_sources)-1)
xlabel('time [s]');
ylabel('rD_b-1')
title('Fractional Change in Blood Flow')